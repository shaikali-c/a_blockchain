...
Block::Block(Hash prev, std::vector<Transaction> txs, uint64_t ts, uint32_t nonce)
    : header_{prev, {}, ts, nonce, 1}, transactions(std::move(txs)) {
...
std::ostream& operator<<(std::ostream& os, const BlockHeader& hdr) {
    ...
    os << "\xe2\x94\x94\xe2\x94\x80 Version:      " << hdr.version;
    return os;
}
...
std::ostream& operator<<(std::ostream& os, const Block& blk) {
    ...
    os << "\xe2\x94\x9c\xe2\x94\x80 Version:      " << blk.header().version << "\n";
